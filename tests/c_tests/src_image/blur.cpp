...
void blur(byte * input, byte * output, int height, int width, int fields){
	...
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++){
			...
			if ( (i >= radius/2) && (j >= radius/2) && (i < height - radius/2) && (j < width - radius/2) ){
				for (int x = i - radius / 2; x <= i + radius / 2; x++){
					for (int y = j - radius / 2; y <= j + radius / 2; y++){
						for (int k = 0; k < fields; k++){
							output[index + k] += ( (input[(x*width + y)*fields + k]) / (radius * radius) );
						}
					}
				}
