...
int main(int argc, char **argv) {
	...
	Image<uint8_t> input_image = load_image<uint8_t>(argv[1]);
	Func input("input");

	float r_sigma = 4;
	int s_sigma = 5;
	Var x("x"), y("y"), z("z"), c("c");
	input(x, y) = cast<float>(input_image(x, y, 0)) / 255.0;
	...
	Image<uint8_t> out_image = output.realize(input_image.width(), input_image.height());
