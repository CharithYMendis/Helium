...
for %%a in (%PHASE%) do (
	...
	if "%%a" == "profile" (
		set CLIENT_ARGUMENTS=!CLIENT_ARGUMENTS! -profile %FILTER_FILE% %FILTER_MODE% %EXALGO_OUTPUT_FOLDER%
	)
	if "%%a" == "memtrace" (
		set CLIENT_ARGUMENTS=!CLIENT_ARGUMENTS! -memtrace %FILTER_FILE% %FILTER_MODE% %EXALGO_OUTPUT_FOLDER%
 	)
	if "%%a" == "funcwrap" (
	...
	if "%%a" == "instrace" (
		set CLIENT_ARGUMENTS=!CLIENT_ARGUMENTS! -instrace %FILTER_FILE% %FILTER_MODE% %EXALGO_OUTPUT_FOLDER% 300000 %INSTRACE_MODE%
	)
	if "%%a" == "memdump" (
		set CLIENT_ARGUMENTS=!CLIENT_ARGUMENTS! -memdump %FILTER_FILE% %FILTER_MODE% %EXALGO_OUTPUT_FOLDER%
	)
)
